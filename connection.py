...
class DadosEscriturais:
    ...
    def BuscarGIM(cgf, data_inicio, data_fim):
        ...
        with oracledb.connect(user=user, password=pw, dsn=dns_tns) as connection:
            gim = pd.read_sql_query(query, con=connection)
...
class DadosCadastrais:
    ...
    def BuscarCadastroHistorico(cnpj):
        ...
        cadastro_historico = extrair_df(query)
...
class DadosDebitos:
    ...
    def BuscarDsot(cgf, data_inicio, data_fim):
    ...
    def BuscarJustificativa(codigos: tuple):
        ...
        query = f""" SELECT
                ...
                """
        return extrair_df_de_lobs(query)
    
    def BuscarDebitosAntecipacaoParcialPagos(cgf, data_inicio, data_fim):
        query = f"""    
            SELECT 
                y.CONINSEST       AS CGF,
...
class DadosOperacoes:
    ...
    def BuscarNotasFiscaisEntrada(cnpj, data_inicio, data_fim):
        ...
        df = extrair_df_de_lobs(query)
        for col, tipo in dtypes.items():
            df[col] = df[col].astype(tipo)
        return df
    
    def BuscarNotasFiscaisExtemporaneas(chaves_acesso):
        ...
        while inicial < len(chaves_acesso):
            ...
            for col, tipo in dtypes.items():
                temp[col] = temp[col].astype(tipo)

            df = pd.concat([df, temp])
            inicial += 1000
        return df 
    
    def BuscarNotasFiscaisSaida(cnpj, data_inicio, data_fim):
    ...
    def BuscarNotasFiscaisConsumidor(cnpj, data_inicio, data_fim):
    ...
    def BuscarNotasFiscaisConsumidorExtemporaneas(chaves_acesso):
    ...
    def BuscarEventosNotas(chaves_acesso):
    ...
    def BuscarEventosNotasConsumidor(chaves_acesso):
    ...
    def BuscarEventosDesembaraco(chaves_acesso):
        inicial = 0
        df = pd.DataFrame()
