...
############################## BLOCO 9 ##############################
        ...
        if line.startswith('|H005|'):
            registro = RegistroH005(
                ...
                REG=campo[0],
                DT_INV=convert_to_datetime(campo[1]),
                VL_INV=replace_comma_with_dot(campo[2]),
                MOT_INV=campo[3]
            )
            session.add(registro)
            id_counters['H005'] += 1
        ...
        if line.startswith('|H010|'):
            registro = RegistroH010(
                ...
                UNID=campo[2],
                QTD=replace_comma_with_dot(campo[3]),
                VL_UNIT=replace_comma_with_dot(campo[4]),
                VL_ITEM=replace_comma_with_dot(campo[5]),
                IND_PROP=campo[6],
                COD_PART=campo[7],
                TXT_COMPL=campo[8],
                COD_CTA=campo[9],
                VL_ITEM_IR=replace_comma_with_dot(campo[10])
            )
            session.add(registro)
            id_counters['H010'] += 1
