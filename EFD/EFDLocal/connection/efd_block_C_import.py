...
def import_efd_C100(efd_id_C001, nfe_only=False) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C100 = pd.DataFrame(results, columns=columns.keys())
    return efd_C100
...
def import_efd_C101(efd_id_C100) -> pd.DataFrame:
    ...
    efd_C101 = pd.DataFrame(results, columns=columns.keys())
    return efd_C101
...
def import_efd_C105(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C105 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C110(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C110 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C111(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C111 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C112(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C112 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C113(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C113 = pd.DataFrame(results, columns=columns.keys())

    # Convert DT_DOC to datetime format
...
def import_efd_C114(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C114 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C115(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C115 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C116(efd_id_C110) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C116 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C120(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C120 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C130(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C130 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C160(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C160 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C165(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C165 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C170(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C170 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C171(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C171 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C172(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C172 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C173(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C173 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C174(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C174 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C175(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C175 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C176(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C176 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C177(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C177 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C178(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C178 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C179(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C179 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C180(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C180 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C181(efd_id_C170) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C181 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C185(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C185 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C186(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C186 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C190(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C190 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C191(efd_id_C190) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C191 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C195(efd_id_C100) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C195 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C197(efd_id_C195) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C197 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C500(efd_id_C001) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C500 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C510(efd_id_C500) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C510 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C590(efd_id_C500) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C590 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C591(efd_id_C590) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C591 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C595(efd_id_C500) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C595 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C597(efd_id_C595) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C597 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C600(efd_id_C001) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C600 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C601(efd_id_C600) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C601 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C610(efd_id_C600) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C610 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C690(efd_id_C600) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C690 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C700(efd_id_C001) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C700 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C790(efd_id_C700) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C790 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C791(efd_id_C790) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C791 = pd.DataFrame(results, columns=columns.keys())
...
def import_efd_C990(efd_id_0000) -> pd.DataFrame:
    ...
    results = query.all()
    efd_C990 = pd.DataFrame(results, columns=columns.keys())
